...
// begin function to create sum of square of gaussian
void sum_square_gen(char *str, int count, int len)
{
  FILE *fp;
  fp = fopen(str, "w");
  int i,j;
  for (i = 0; i < len; i++)
  {
    ...
    for (j = 0; j < count; j++)
    {
      temp = pow(gauss_square(), 2);
    }
    fprintf(fp, "%lf\n", temp);
  }
  fclose(fp);
}
// end function to create sum of square of gaussian

double gauss_71(double var){
    double x=0;
        for (int j = 0; j < 12; j++){
...
// begin function to create data for 7_1
void gen_7_1(char *str,  double gamma){
  FILE *fp, *fp2;
  fp = fopen(str, "w");
  fp2 = fopen("../Data/5_1.dat", "r");
  double bv = 0;
  while(fscanf(fp2,"%lf",&bv)!=EOF){
    double v = gamma/2;
    double a = gauss_71(v);
    double b = gauss_71(v);
  
    double y = bv*sqrt(a*a + b*b) + gauss_71(1);
    fprintf(fp, "%lf\n", y);
  }
  fclose(fp);
  return;
